...
OBJECTIVE:
- Mental Status Exam: [ONLY what you directly observed - appearance, behavior, speech, thought patterns, NO diagnoses or interpretations]
- Physical Observations: [ONLY physical appearance, vital signs if noted, NO medical conclusions]
- Behavioral Observations: [ONLY what you witnessed during session - actions, expressions, movements, NO analysis]

CRITICAL OBJECTIVE RULES:
1. NEVER include: diagnoses, risk levels, treatment plans, safety measures, recommendations
2. ONLY include: what you can see, hear, or directly observe
3. Use descriptive language: "appeared sad", "was crying", "spoke quietly"
4. NO clinical terms like "suicidal ideation", "depression", "risk", "intervention"
5. Focus on: appearance, behavior, speech patterns, body language, emotional expression
...
Please provide a comprehensive analysis including a session summary, patient information (gender, age, present illness history, past history), risk assessment, structured clinical note in SOAP format (SESSION SUMMARY, PATIENT INFORMATION, OBJECTIVE, ASSESSMENT, PLAN), key findings, and follow-up questions. Format the response as a clean clinical note without any asterisks, bold formatting, or special characters.`,
    ...
    try {
      ...
      if (cleanedText.includes('SESSION SUMMARY:')) {
        sessionSummary =
          cleanedText
            .split('SESSION SUMMARY:')[1]
            ?.split('PATIENT INFORMATION:')[0] ||
      ...
      if (hasSafetyConcerns) {
        ...
        if (
          ...
          textLower.includes('risk: high')
        ) {
          riskLevel = 'high';
        } else if (
          textLower.includes('low risk') ||
          textLower.includes('risk: low')
        ) {
          riskLevel = 'low';
        } else if (
          textLower.includes('medium risk') ||
          textLower.includes('risk: medium')
        ) {
          riskLevel = 'medium';
        }
        ...
        if (textLower.includes('suicidal') || textLower.includes('suicide')) {
          ...
          textLower.includes('self-harm') ||
          textLower.includes('cutting')
        ) {
          category = 'Self-Harm';
        } else if (
          textLower.includes('violence') ||
          textLower.includes('homicidal')
        ) {
          category = 'Violence Risk';
        } else if (
          textLower.includes('substance abuse') ||
          textLower.includes('alcohol') ||
          textLower.includes('drugs')
        ) {
        ...
        riskFlags.push({
          ...
          description:
            'No safety concerns identified in this session. Patient presents with normal mood and behavior.',
        });
      }
      ...
      if (
        !textLower.includes('gender') &&
        !textLower.includes('male') &&
        !textLower.includes('female') &&
        !textLower.includes('non-binary')
      ) {
      ...
      if (
        !textLower.includes('present illness') &&
        !textLower.includes('current symptoms') &&
        !textLower.includes('symptoms started') &&
        !textLower.includes('timeline')
      ) {
        missingInfoQuestions.push(
          'Can you provide a detailed timeline of when the current symptoms began and how they have progressed?',
        );
      }

      // Check for missing past psychiatric history
      if (
        !textLower.includes('past psychiatric') &&
        !textLower.includes('previous diagnosis') &&
        !textLower.includes('prior treatment') &&
        !textLower.includes('hospitalization')
      ) {
        missingInfoQuestions.push(
          'Does the patient have any previous psychiatric diagnoses, treatments, or hospitalizations?',
        );
      }

      // Check for missing past medical history
      if (
        !textLower.includes('past medical') &&
        !textLower.includes('medical conditions') &&
        !textLower.includes('medications') &&
        !textLower.includes('surgeries')
      ) {
        missingInfoQuestions.push(
          "What is the patient's past medical history, including current medications, medical conditions, or surgeries?",
        );
      }

      // Check for missing family history
      if (
        !textLower.includes('family history') &&
        !textLower.includes('family mental health') &&
        !textLower.includes('parents') &&
        !textLower.includes('siblings')
      ) {
        missingInfoQuestions.push(
          'Is there any family history of mental health conditions, substance abuse, or psychiatric disorders?',
        );
      }

      // Check for missing social history
      if (
        !textLower.includes('social history') &&
        !textLower.includes('occupation') &&
        !textLower.includes('living situation') &&
        !textLower.includes('relationships') &&
        !textLower.includes('substance use')
      ) {
        missingInfoQuestions.push(
          "What is the patient's social history including occupation, living situation, relationships, and any substance use?",
        );
      }
      ...
      if (hasSafetyConcerns) {
        if (
          !textLower.includes('suicidal ideation') &&
          !textLower.includes('suicide') &&
          !textLower.includes('kill myself')
        ) {
          missingInfoQuestions.push(
            'Has the patient expressed any suicidal thoughts, plans, or intent? If so, what are the specific details?',
          );
        }
        ...
        if (
          !textLower.includes('self-harm') &&
          !textLower.includes('cutting') &&
          !textLower.includes('self-injury')
        ) {
          missingInfoQuestions.push(
            'Has the patient engaged in any self-harm behaviors? If so, what methods and frequency?',
          );
        }
        ...
        if (
          !textLower.includes('violence') &&
          !textLower.includes('homicidal') &&
          !textLower.includes('harm others')
        ) {
          missingInfoQuestions.push(
            'Has the patient expressed any violent thoughts or intentions toward others?',
          );
        }
        ...
        if (
          ...
          missingInfoQuestions.push(
            "What is the patient's current and past substance use history, including alcohol, drugs, and prescription medications?",
          );
        }
      }
      ...
      if (!textLower.includes('mental status') && !textLower.includes('mse')) {
        missingInfoQuestions.push(
          'Can you provide a detailed Mental Status Examination including appearance, behavior, mood, affect, speech, thought process, thought content, perception, cognition, and insight?',
        );
      }
      ...
      if (
        ...
        missingInfoQuestions.push(
          'What are your physical observations of the patient including appearance, behavior, and any notable physical signs?',
        );
      }
      ...
      if (
        ...
        missingInfoQuestions.push(
          'What specific treatment recommendations or interventions are you considering for this patient?',
        );
      }
      ...
      // Extract objective, assessment and plan content with proper categorization
      let objectiveContent = '';
      let assessmentContent = '';
      let planContent = '';

      // Extract objective section - look for various patterns
      if (cleanedText.includes('OBJECTIVE:')) {
        objectiveContent =
          cleanedText.split('OBJECTIVE:')[1]?.split('ASSESSMENT:')[0] || '';
      } else if (cleanedText.includes('Objective:')) {
        objectiveContent =
          cleanedText.split('Objective:')[1]?.split('Assessment:')[0] || '';
      }
      ...
      if (cleanedText.includes('ASSESSMENT:')) {
        assessmentContent =
          cleanedText.split('ASSESSMENT:')[1]?.split('PLAN:')[0] || '';
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (
            line.toLowerCase().includes('primary diagnosis') ||
            ...
            inAssessment &&
            (line.toLowerCase().includes('immediate intervention') ||
      ...
      if (cleanedText.includes('PLAN:')) {
        ...
        planContent = cleanedText.split('Plan:')[1] || '';
      } else if (cleanedText.toLowerCase().includes('plan:')) {
        // Handle lowercase "plan:"
        const planIndex = cleanedText.toLowerCase().indexOf('plan:');
        planContent = cleanedText.substring(planIndex + 5) || '';
      } else {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (
            lowerLine.includes('immediate intervention') ||
            lowerLine.includes('treatment recommendation') ||
            lowerLine.includes('follow-up') ||
            lowerLine.includes('safety measure') ||
            lowerLine.includes('contact emergency') ||
            lowerLine.includes('notify family') ||
            lowerLine.includes('provide crisis') ||
            lowerLine.includes('establish') ||
            lowerLine.includes('arrange') ||
            lowerLine.includes('psychiatric evaluation') ||
            lowerLine.includes('inpatient admission')
          ) {
            inPlan = true;
            planLines.push(line);
          } else if (
            inPlan &&
            line &&
            !lowerLine.includes('risk assessment') &&
            !lowerLine.includes('assessment:') &&
      ...
      if (assessmentContent) {
        const assessmentLines = assessmentContent
          .split('\n')
        ...
        assessmentLines.forEach((line) => {
          ...
          if (lowerLine.includes('primary diagnosis')) {
            ...
            assessmentCategories.primaryDiagnosis =
              line.split(':')[1]?.trim() ||
              line.replace(/primary diagnosis/i, '').trim();
          } else if (lowerLine.includes('differential diagnosis')) {
            currentCategory = 'differentialDiagnoses';
            assessmentCategories.differentialDiagnoses =
              line.split(':')[1]?.trim() ||
              line.replace(/differential diagnosis/i, '').trim();
          } else if (
            lowerLine.includes('risk assessment') ||
            lowerLine.includes('risk high') ||
            lowerLine.includes('risk medium') ||
            lowerLine.includes('risk low')
          ) {
      ...
      if (planContent) {
        ...
        planLines.forEach((line) => {
          ...
          // Categorize based on action type and urgency
          if (
            lowerLine.includes('immediate intervention') ||
            lowerLine.includes('contact emergency') ||
            lowerLine.includes('psychiatric evaluation') ||
            lowerLine.includes('inpatient admission') ||
            lowerLine.includes('emergency services')
          ) {
            currentCategory = 'immediateInterventions';
            if (!planCategories.immediateInterventions) {
              planCategories.immediateInterventions = trimmedLine;
            } else {
              planCategories.immediateInterventions += `. ${trimmedLine}`;
            }
          } else if (
            lowerLine.includes('treatment recommendation') ||
            lowerLine.includes('therapy') ||
            lowerLine.includes('medication') ||
            lowerLine.includes('treatment plan')
          ) {
            currentCategory = 'treatmentRecommendations';
            if (!planCategories.treatmentRecommendations) {
              planCategories.treatmentRecommendations = trimmedLine;
            } else {
              planCategories.treatmentRecommendations += `. ${trimmedLine}`;
            }
          } else if (
            lowerLine.includes('follow-up') ||
            lowerLine.includes('daily until') ||
            lowerLine.includes('arrange') ||
            lowerLine.includes('schedule')
          ) {
            currentCategory = 'followUpPlan';
            if (!planCategories.followUpPlan) {
              planCategories.followUpPlan = trimmedLine;
            } else {
              planCategories.followUpPlan += `. ${trimmedLine}`;
            }
          } else if (
            lowerLine.includes('safety measure') ||
            lowerLine.includes('notify family') ||
            lowerLine.includes('remove access') ||
            lowerLine.includes('crisis hotline') ||
            lowerLine.includes('support plan') ||
            lowerLine.includes('remove medications')
          ) {
            currentCategory = 'safetyMeasures';
            if (!planCategories.safetyMeasures) {
              planCategories.safetyMeasures = trimmedLine;
            } else {
              planCategories.safetyMeasures += `. ${trimmedLine}`;
            }
          } else if (currentCategory && trimmedLine) {
            // Append to current category if we're in one and it's a continuation
      ...
      // Clean and categorize objective content
      ...
      if (objectiveContent) {
        const objectiveLines = objectiveContent
          .split('\n')
          .filter((line) => line.trim());
        let currentCategory = '';

        // Define terms that should NEVER appear in objective content
        ...
        objectiveLines.forEach((line) => {
          ...
          // Skip lines that contain prohibited terms
          ...
          // Skip lines that look like assessment/plan bullets
          if (
            lowerLine.startsWith('- primary') ||
            lowerLine.startsWith('- differential') ||
            lowerLine.startsWith('- risk') ||
            lowerLine.startsWith('- treatment') ||
            lowerLine.startsWith('- immediate') ||
            lowerLine.startsWith('- safety') ||
            lowerLine.startsWith('- follow')
          ) {
            return; // Skip assessment/plan bullets
          }

          if (lowerLine.includes('mental status exam')) {
            currentCategory = 'mentalStatusExam';
            let content =
              line.split(':')[1]?.trim() ||
              line.replace(/mental status exam/i, '').trim();
            ...
            if (
              ...
              objectiveCategories.mentalStatusExam = content;
            }
          } else if (lowerLine.includes('physical observations')) {
            currentCategory = 'physicalObservations';
            let content =
              line.split(':')[1]?.trim() ||
              line.replace(/physical observations/i, '').trim();
            ...
            if (
              ...
              objectiveCategories.physicalObservations = content;
            }
          } else if (lowerLine.includes('behavioral observations')) {
            currentCategory = 'behavioralObservations';
            let content =
              line.split(':')[1]?.trim() ||
              line.replace(/behavioral observations/i, '').trim();
            content = content.replace(/^[-•]\s*/, '');
            if (
              content &&
              !prohibitedTerms.some((term) =>
              ...
              objectiveCategories.behavioralObservations = content;
            }
          } else if (currentCategory && trimmedLine && !containsProhibited) {
            // Only append if it doesn't contain prohibited terms
            ...
            if (
              ...
              if (currentCategory === 'mentalStatusExam') {
                objectiveCategories.mentalStatusExam += ` ${cleanLine}`;
              } else if (currentCategory === 'physicalObservations') {
                objectiveCategories.physicalObservations += ` ${cleanLine}`;
              } else if (currentCategory === 'behavioralObservations') {
                objectiveCategories.behavioralObservations += ` ${cleanLine}`;
              }
            }
          }
        });

        // Final cleanup - remove any remaining prohibited content
        (
          Object.keys(objectiveCategories) as Array<
            keyof typeof objectiveCategories
          >
        ).forEach((key) => {
          const content = objectiveCategories[key];
          if (content) {
            // Split by sentence and filter out sentences with prohibited terms
            const sentences = content
              ...
              .filter((sentence: string) => {
                ...
                return (
                  cleanSentence &&
                  !prohibitedTerms.some((term) =>
                    cleanSentence.toLowerCase().includes(term),
                  )
                );
              });
            objectiveCategories[key] = sentences.join('. ').trim();
          }
        });
      }
      ...
      clinicalNote = {
        ...clinicalNote,
        objective: {
          content: objectiveContent.trim() || 'No objective data available.',
    ...
    const safeResponse = {
      ...
      objective: clinicalNote.objective || {
        content: 'No objective data available.',
        categories: {
          mentalStatusExam: '',
          physicalObservations: '',
          behavioralObservations: '',
    ...
    return NextResponse.json(
      {
        ...
        flaggedWords: [],
        objective: {
          content: 'Unable to process objective due to technical error.',
          categories: {
            mentalStatusExam: '',
            physicalObservations: '',
            behavioralObservations: '',
